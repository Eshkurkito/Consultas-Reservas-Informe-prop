...
# ================== BLOQUE INLINE — INFORME PROPIETARIO (PDF, PRO) ==================
...
def _save_matplotlib_chart(make_plot_fn, out_path: str):
    """Dibuja un gráfico con matplotlib y lo guarda en PNG. (No usa seaborn)."""
    try:
...
def _build_daily_occupancy(df: pd.DataFrame, period_start, period_end, inv: int) -> pd.DataFrame:
    import pandas as _pd
    days = _pd.date_range(start=_pd.to_datetime(period_start), end=_pd.to_datetime(period_end), freq="D")
    occ = _pd.Series(0.0, index=days)
    pe_exclusive = _pd.to_datetime(period_end) + _pd.Timedelta(days=1)
    for _, r in df.iterrows():
        r_in, r_out = _pd.to_datetime(r["_in"]), _pd.to_datetime(r["_out"])
        start = max(r_in, _pd.to_datetime(period_start))
        end   = min(r_out, pe_exclusive)
        if end <= start:
            continue
        span = _pd.date_range(start=start, end=end - _pd.Timedelta(days=1), freq="D")
        occ.loc[occ.index.isin(span)] += 1.0
    inv = max(int(inv), 1)
    occ_pct = (occ / inv) * 100.0
    return _pd.DataFrame({"Ocupación %": occ_pct})

def _owner_payout_summary(revenue: float, mgmt_pct: float, fixed_costs: float) -> dict:
    mgmt_fee = revenue * (mgmt_pct / 100.0)
    payout = revenue - mgmt_fee - fixed_costs
    return {"mgmt_fee": mgmt_fee, "fixed_costs": fixed_costs, "payout": payout}


def render_owner_report_mode():
    st.header("Informe de Propietario (PDF) — PRO+")
    raw = st.session_state.get("raw_df")

    if raw is None or not isinstance(raw, pd.DataFrame):
        st.warning("No hay datos. Sube un CSV/Excel en la barra lateral.")
        return
    if raw.empty:
        st.warning("El DataFrame está vacío.")
        return

    with st.expander("Vista previa de datos (primeras 12 filas)", expanded=False):
    ...
    with st.sidebar:
        ...
        # Detección de columnas y selección manual
        def _auto_detect_columns(df: pd.DataFrame) -> dict:
            guess = {"aloj": None, "ini": None, "fin": None, "alta": None, "rev": None, "price": None, "canal": None}
            cols = list(df.columns)

            aloj_patterns = ["Alojamiento","Unidad","Apartamento","Propiedad","Listing","Unidad/Alojamiento","Nombre alojamiento","Unit","Property","Apartment","Listing Name"]
            for c in cols:
                for pat in aloj_patterns:
                    if pat.lower() in str(c).lower():
                        guess["aloj"] = c; break
                if guess["aloj"]: break

            ini_patterns = ["Fecha entrada","Entrada","Check-in","Fecha de entrada","Arrival","Arrival date","Check in"]
            fin_patterns = ["Fecha salida","Salida","Check-out","Fecha de salida","Departure","Departure date","Check out"]
            for c in cols:
                if not guess["ini"] and any(pat.lower() in str(c).lower() for pat in ini_patterns):
                    guess["ini"] = c
                if not guess["fin"] and any(pat.lower() in str(c).lower() for pat in fin_patterns):
                    guess["fin"] = c
                if guess["ini"] and guess["fin"]:
                    break

            alta_patterns = ["Fecha alta","Creación","Fecha reserva","Booking Date","Fecha de creación","Created"]
            for c in cols:
                if any(pat.lower() in str(c).lower() for pat in alta_patterns):
                    guess["alta"] = c; break

            rev_patterns = ["ingres","revenue","importe","total","aloj","payout","amount","net","bruto","gross"]
            for c in cols:
                if any(pat in str(c).lower() for pat in rev_patterns):
                    try:
                        if pd.api.types.is_datetime_dtype(df[c]):
                            continue
                    except Exception:
                        pass
                    guess["rev"] = c; break

            price_patterns = ["precio","tarifa","rate","adr","precio por noche","nightly"]
            for c in cols:
                if any(pat in str(c).lower() for pat in price_patterns):
                    guess["price"] = c; break

            canal_patterns = ["canal","source","origen","channel","booking channel"]
            for c in cols:
                if any(p in str(c).lower() for p in canal_patterns):
                    guess["canal"] = c; break

            return guess

        guess = _auto_detect_columns(raw)

        aloj_col = st.selectbox(
            "Columna de alojamiento",
            options=list(raw.columns),
            index=(list(raw.columns).index(guess["aloj"]) if guess["aloj"] in raw.columns else 0)
        )
        ini_col = st.selectbox(
            "Columna fecha entrada",
            options=list(raw.columns),
            index=(list(raw.columns).index(guess["ini"]) if guess["ini"] in raw.columns else 0)
        )
        fin_col = st.selectbox(
            "Columna fecha salida",
            options=list(raw.columns),
            index=(list(raw.columns).index(guess["fin"]) if guess["fin"] in raw.columns else 0)
        )

        alta_col = st.selectbox(
            "Columna fecha alta (opcional)",
            options=["(Ninguna)"] + list(raw.columns),
            index=(list(raw.columns).index(guess["alta"])+1 if guess["alta"] in raw.columns else 0)
        )
        alta_col = None if alta_col == "(Ninguna)" else alta_col

        num_cols = [c for c in raw.columns if pd.api.types.is_numeric_dtype(raw[c])]
        rev_default_idx = num_cols.index(guess["rev"]) if guess["rev"] in num_cols else 0 if num_cols else 0
        price_default_idx = num_cols.index(guess["price"]) if guess["price"] in num_cols else (0 if num_cols else 0)

        rev_col = st.selectbox(
            "Columna ingresos (opcional)",
            options=["(Ninguna)"] + num_cols,
            index=(rev_default_idx+1 if num_cols and guess["rev"] in num_cols else 0)
        )
        rev_col = None if rev_col == "(Ninguna)" else rev_col

        price_col = st.selectbox(
            "Columna precio por noche (opcional, si no hay ingresos)",
            options=["(Ninguna)"] + num_cols,
            index=(price_default_idx+1 if num_cols and guess["price"] in num_cols else 0)
        )
        price_col = None if price_col == "(Ninguna)" else price_col

        canal_col = st.selectbox(
            "Columna canal (opcional)",
            options=["(Ninguna)"] + list(raw.columns),
            index=(list(raw.columns).index(guess["canal"])+1 if guess["canal"] in list(raw.columns) else 0)
        )
        canal_col = None if canal_col == "(Ninguna)" else canal_col

        try:
            prop_options = sorted([str(x) for x in raw[aloj_col].dropna().unique().tolist()])
        except Exception:
            prop_options = []
        ...
        mgmt_pct   = st.number_input("Comisión de gestión (%)", min_value=0.0, max_value=100.0, value=20.0, step=0.5)
        fixed_costs = st.number_input("Costes fijos del periodo (€)", min_value=0.0, value=0.0, step=50.0)
    ...
    def _kpis_from_df(
        ...
        aloj_col: str | None = None,
        ini_col: str | None = None,
        fin_col: str | None = None,
        alta_col: str | None = None,
        rev_col: str | None = None,
        price_col: str | None = None,
    ):
        df = df_all.copy()

        if aloj_col and aloj_col in df.columns and aloj_col != "Alojamiento":
            df = df.rename(columns={aloj_col: "Alojamiento"})
        if ini_col and ini_col in df.columns and ini_col != "_EntradaTmp":
            df = df.rename(columns={ini_col: "_EntradaTmp"})
        if fin_col and fin_col in df.columns and fin_col != "_SalidaTmp":
            df = df.rename(columns={fin_col: "_SalidaTmp"})
        if alta_col and alta_col in df.columns and alta_col != "_AltaTmp":
            df = df.rename(columns={alta_col: "_AltaTmp"})
        if rev_col and rev_col in df.columns and rev_col != "_RevTmp":
            df = df.rename(columns={rev_col: "_RevTmp"})
        if price_col and price_col in df.columns and price_col != "_PriceTmp":
            df = df.rename(columns={price_col: "_PriceTmp"})

        if "Alojamiento" not in df.columns:
            raise ValueError("Selecciona la columna de alojamiento.")
        if "_EntradaTmp" not in df.columns or "_SalidaTmp" not in df.columns:
            raise ValueError("Selecciona columnas de entrada/salida.")

        if "_AltaTmp" in df.columns:
            df = df[pd.to_datetime(df["_AltaTmp"], errors="coerce") <= pd.to_datetime(cutoff)]

        if filter_props:
            df = df[df["Alojamiento"].astype(str).isin(filter_props)]

        df["_in"]  = _parse_date_series(df["_EntradaTmp"])
        df["_out"] = _parse_date_series(df["_SalidaTmp"])
        ...
        revenue = 0.0
        agg = {}
        pe_exclusive = pd.to_datetime(period_end) + pd.Timedelta(days=1)

        if "_RevTmp" in df.columns:
            df["_RevTmp"] = pd.to_numeric(df["_RevTmp"], errors="coerce")
        if "_PriceTmp" in df.columns:
            df["_PriceTmp"] = pd.to_numeric(df["_PriceTmp"], errors="coerce")

        df_ci_window = []

        for _, row in df.iterrows():
            r_in, r_out = pd.to_datetime(row["_in"]), pd.to_datetime(row["_out"])
            start = max(r_in, pd.to_datetime(period_start))
            end   = min(r_out, pe_exclusive)
            n = (end - start).days
            if n <= 0:
                continue
            ...
            rev_row = 0.0
            if "_RevTmp" in df.columns and pd.notna(row.get("_RevTmp", float("nan"))):
                total_n = (r_out - r_in).days
                total_n = total_n if total_n > 0 else n
                frac = n / total_n
                try:
                    rev_row = float(row.get("_RevTmp", 0.0)) * frac
                except Exception:
                    rev_row = 0.0
            elif "_PriceTmp" in df.columns and pd.notna(row.get("_PriceTmp", float("nan"))):
                try:
                    rev_row = float(row.get("_PriceTmp", 0.0)) * n
                except Exception:
                    rev_row = 0.0
            ...
            k = str(row["Alojamiento"])
            if k not in agg:
                agg[k] = {"nights": 0, "rev": 0.0}
            agg[k]["nights"] += n
            agg[k]["rev"]    += rev_row

            if period_start <= r_in.date() <= period_end:
                df_ci_window.append(row)
        ...
        extras = {"df_for_charts": df, "inv": inv, "df_ci_window": pd.DataFrame(df_ci_window) if df_ci_window else pd.DataFrame()}
        tot = {
        ...
        return by_prop, tot, extras

    # Ejecutar
    error_msg = None
    try:
        by_prop, tot, extras = _kpis_from_df(
            df_all=raw,
        ...
        error_msg = str(e)
        by_prop, tot, extras = pd.DataFrame(), {
            "noches_ocupadas": 0, "noches_disponibles": 0, "ocupacion_pct": 0.0,
            "ingresos": 0.0, "adr": 0.0, "revpar": 0.0
        }, {"df_for_charts": pd.DataFrame(), "inv": 1, "df_ci_window": pd.DataFrame()}

    with st.expander("Diagnóstico / mapeo de columnas", expanded=False):
        st.write({
            "aloj_col": aloj_col, "ini_col": ini_col, "fin_col": fin_col,
            "alta_col": alta_col, "rev_col": rev_col, "price_col": price_col,
            "canal_col": canal_col
        })
        if error_msg:
            st.error(error_msg)
    ...
    if not by_prop.empty:
        ...
        st.warning("No hay pernoctaciones solapadas con el periodo seleccionado o el mapeo de columnas no coincide.")

    payout = _owner_payout_summary(tot["ingresos"], mgmt_pct, fixed_costs)

    # -------- Generación PDF PRO+ (maquetación mejorada) --------
    try:
        from fpdf import FPDF
    except Exception as e:
        st.error(f"No se pudo importar fpdf2: {e}")
        st.info("Añade a requirements.txt: fpdf2==2.7.9 y matplotlib")
        st.stop()
    ...
    LOGO_PATHS = [
        "/mnt/data/LOGO_CIRCULAR_FLORIT FLATS.png",
        "./assets/logo.png","./assets/logo.jpg","./logo.png","./logo.jpg"
    ]
    logo_path = None
    ...
    def draw_section_title(pdf, text, y=None):
        if y is not None:
            pdf.set_y(y)
        pdf.set_text_color(*BRAND_RGB)
        pdf.set_font(*h2_font)
        pdf.cell(0, 9, _safe_text(text, unicode_ok), ln=1)
    ...
    def draw_kpi_card(pdf, x, y, label, value, subtitle=None):
        pdf.set_fill_color(235, 237, 240)
        pdf.rect(x+1.2, y+1.6, 56, 32, style="F")
        pdf.set_draw_color(225, 228, 232)
        pdf.set_fill_color(255, 255, 255)
        pdf.rect(x, y, 56, 32, style="DF")
        pdf.set_fill_color(*BRAND_RGB)
        pdf.rect(x, y, 56, 5, style="F")
        pdf.set_text_color(*BRAND_RGB)
        pdf.set_xy(x, y+8)
        pdf.set_font(*val_font)
        pdf.cell(56, 8, _safe_text(value, unicode_ok), align="C")
        pdf.set_text_color(90, 90, 90)
        pdf.set_xy(x, y+18)
        pdf.set_font(*tag_font)
        pdf.cell(56, 5, _safe_text(label, unicode_ok), align="C")
        if subtitle:
            pdf.set_xy(x, y+23)
            pdf.set_font(*small_font)
            pdf.set_text_color(130, 130, 130)
            pdf.cell(56, 4, _safe_text(subtitle, unicode_ok), align="C")

    def draw_table_by_prop(pdf, df):
        pdf.set_font(*h3_font)
        pdf.set_text_color(*BRAND_RGB)
        pdf.cell(0, 8, _safe_text("Detalle por alojamiento", unicode_ok), ln=1)
        pdf.ln(1)
        headers = ["Alojamiento", "Noches", "Ingresos (€)", "ADR (€)"]
        widths  = [90, 25, 35, 25]
        pdf.set_fill_color(22, 62, 100)
        pdf.set_text_color(255, 255, 255)
        pdf.set_font(*h3_font)
        for h, w in zip(headers, widths):
            pdf.cell(w, 8, _safe_text(h, unicode_ok), border=0, align="L", fill=True)
        pdf.ln(8)
        ...
        for _, row in df.iterrows():
            ...
            pdf.cell(widths[0], 7, _safe_text(str(row["Alojamiento"]), unicode_ok), fill=True)
            pdf.cell(widths[1], 7, _safe_text(f"{int(row['Noches ocupadas'])}", unicode_ok), align="R", fill=True)
            pdf.cell(widths[2], 7, _safe_text(f"{row['Ingresos (€)']:.2f}", unicode_ok), align="R", fill=True)
            pdf.cell(widths[3], 7, _safe_text(f"{row['ADR (€)']:.2f}", unicode_ok), align="R", fill=True)
            pdf.ln(7)
            fill = not fill

    def new_page_if_needed(pdf, extra_height=40):
        if pdf.get_y() + extra_height > 270:
            pdf.add_page()

    class OwnerReportPro(FPDF):
        def header(self):
    ...
    pdf = OwnerReportPro(orientation="P", unit="mm", format="A4")
    ...
    if not unicode_ok:
        ...
        h3_font    = ("Helvetica", "B", 12)
        body_font  = ("Helvetica", "", 11)
        small_font = ("Helvetica", "", 9)
        val_font   = ("Helvetica", "B", 15)
        tag_font   = ("Helvetica", "", 9)

    # ---------- PORTADA ----------
    pdf.add_page()
    pdf.set_fill_color(237, 242, 247)
    pdf.rect(15, 15, 180, 22, style="F")
    if logo_path:
        try:
            pdf.image(logo_path, x=170, y=17, w=20)
        except Exception:
            pass
    pdf.set_xy(20, 20)
    pdf.set_text_color(*BRAND_RGB)
    pdf.set_font(*h3_font)
    pdf.cell(0, 7, _safe_text("Florit Flats — Informe de Propietario", unicode_ok), ln=1)
    pdf.set_text_color(60, 60, 60)
    pdf.set_font(*small_font)
    pdf.cell(0, 5, _safe_text("Resumen mensual de desempeño y liquidación", unicode_ok), ln=1)
    pdf.set_text_color(*BRAND_RGB)
    pdf.set_font(*title_font)
    pdf.set_xy(20, 50)
    pdf.cell(0, 10, _safe_text(mes_text, unicode_ok), ln=1)
    pdf.set_text_color(80, 80, 80)
    pdf.set_font(*body_font)
    pdf.set_xy(20, 65)
    pdf.cell(85, 7, _safe_text(f"Propietario: {propietario}", unicode_ok), ln=0)
    pdf.cell(85, 7, _safe_text(f"Propiedad: {propiedad}", unicode_ok), ln=1)
    pdf.cell(85, 7, _safe_text(f"Generado: {pd.Timestamp.today().strftime('%d %b %Y')}", unicode_ok), ln=0)
    pdf.cell(85, 7, _safe_text(f"Periodo: {p_start.strftime('%d/%m/%Y')} – {p_end.strftime('%d/%m/%Y')}", unicode_ok), ln=1)
    pdf.set_draw_color(225, 228, 232)
    pdf.set_fill_color(245, 247, 250)
    pdf.rect(20, 82, 170, 52, style="DF")
    pdf.set_xy(20, 98)
    pdf.set_text_color(110, 110, 110)
    pdf.set_font(*tag_font)
    pdf.cell(170, 6, _safe_text("Espacio para fotografía (Canva) o gráfico destacado (PNG)", unicode_ok), align="C", ln=1)

    # ---------- RESUMEN EJECUTIVO ----------
    pdf.add_page()
    draw_section_title(pdf, "Resumen ejecutivo", y=20)
    pdf.set_text_color(65, 65, 65)
    pdf.set_font(*body_font)
    resumen = (
        f"Durante {mes_text}, la ocupación alcanzó el {tot['ocupacion_pct']:.2f}%. "
        f"El ADR fue de {tot['adr']:.2f} € y el RevPAR de {tot['revpar']:.2f} €, "
        f"con unos ingresos brutos de {tot['ingresos']:.2f} €. "
        "Estos resultados se han visto influenciados por la demanda del periodo, la estrategia de precios "
        "y el mix de canales. A continuación, se muestran los indicadores clave y recomendaciones tácticas."
    )
    pdf.multi_cell(0, 6, _safe_text(resumen, unicode_ok))
    pdf.ln(2)
    ...
    for i, (lab, val) in enumerate(kpis):
        ...
        x = x0 + col * (56 + gx)
        y = y0 + row * (32 + gy)
        draw_kpi_card(pdf, x, y, lab, val)

    pdf.set_y(70 + 2*(32 + gy) + 4)
    pdf.set_font(*h3_font)
    pdf.set_text_color(*BRAND_RGB)
    pdf.cell(0, 8, _safe_text("Recomendaciones tácticas", unicode_ok), ln=1)
    pdf.set_text_color(70, 70, 70)
    pdf.set_font(*body_font)
    pdf.multi_cell(0, 6, _safe_text(
        "- Ajustar precios de alta demanda con 30–45 días de antelación.\n"
        "- Mantener mínimos de estancia (LOS) en fines de semana y eventos.\n"
        "- Reforzar venta directa cuando el canal dominante tenga comisiones elevadas.", unicode_ok))

    # ---------- VISUALIZACIONES ----------
    cur_y = pdf.get_y() + 6
    try:
        df_for_ch = extras.get("df_for_charts", pd.DataFrame())
        inv_for_ch = extras.get("inv", 1)
        if not df_for_ch.empty:
            daily = _build_daily_occupancy(df_for_ch, p_start, p_end, inv_for_ch)
            chart1 = _tmp_png("owner_daily_occ.png")
            def _plot1(plt, ax):
                daily.plot(ax=ax, legend=False)
                ax.grid(True, alpha=0.25)
                ax.set_title("Ocupación diaria (%)")
                ax.set_xlabel("Fecha"); ax.set_ylabel("%")
            if _save_matplotlib_chart(_plot1, chart1):
                new_page_if_needed(pdf, 80)
                pdf.image(chart1, x=20, y=cur_y, w=170)
                cur_y += 78
    except Exception:
        pass

    try:
        if canal_col and canal_col in raw.columns:
            df_ci = extras.get("df_ci_window", pd.DataFrame())
            if not df_ci.empty:
                mix = df_ci[canal_col].astype(str).value_counts().head(8)
                chart2 = _tmp_png("owner_channel_mix.png")
                def _plot2(plt, ax):
                    mix.plot(kind="bar", ax=ax)
                    ax.grid(True, axis="y", alpha=0.25)
                    ax.set_title("Mix de canales (check-ins del periodo)")
                    ax.set_xlabel("Canal"); ax.set_ylabel("Reservas")
                if _save_matplotlib_chart(_plot2, chart2):
                    new_page_if_needed(pdf, 80)
                    pdf.image(chart2, x=20, y=cur_y, w=170)
                    cur_y += 78
    except Exception:
        pass
    ...
    pdf.add_page()
    draw_section_title(pdf, "Resumen económico del propietario", y=20)
    pdf.set_text_color(65, 65, 65)
    pdf.set_font(*body_font)
    explain = (
        "El ingreso bruto corresponde al total imputado sobre las noches que caen dentro del periodo. "
        "La comisión de gestión se aplica sobre el ingreso bruto y, a continuación, se restan los costes fijos declarados. "
        "El resultado es el pago neto al propietario."
    )
    pdf.multi_cell(0, 6, _safe_text(explain, unicode_ok))
    pdf.ln(2)

    econ = [
        ("Ingresos brutos (€)", f"{tot['ingresos']:.2f}", None),
        ("Comisión gestión (%)", f"{mgmt_pct:.1f}%", None),
        ("Comisión gestión (€)", f"{(tot['ingresos'] * (mgmt_pct/100)):,.2f}".replace(",", "."), None),
        ("Costes fijos (€)",     f"{fixed_costs:.2f}", None),
        ("Pago propietario (€)", f"{(tot['ingresos'] - (tot['ingresos']*(mgmt_pct/100)) - fixed_costs):,.2f}".replace(",", "."), "Liquidez estimada"),
    ]
    x0, y0 = 20, pdf.get_y() + 4
    for i, (lab, val, sub) in enumerate(econ):
        col = i % 3
        row = i // 3
        x = x0 + col * (56 + gx)
        y = y0 + row * (32 + gy)
        draw_kpi_card(pdf, x, y, lab, val, sub)

    if not by_prop.empty:
        new_page_if_needed(pdf, 80)
        pdf.set_y(y0 + 2*(32+gy) + 8)
        draw_table_by_prop(pdf, by_prop)

    # ---------- GLOSARIO ----------
    pdf.add_page()
    draw_section_title(pdf, "Glosario y metodología", y=20)
    pdf.set_text_color(65, 65, 65)
    pdf.set_font(*body_font)
    texto_glosario = (
        "• Ocupación (%): noches vendidas / noches disponibles.\n"
        "• ADR (€): ingreso medio por noche ocupada. Si no hay columna de ingresos, se aproxima como precio/noche × noches.\n"
        "• RevPAR (€): ingresos / noches disponibles.\n"
        "• Fecha de corte: si existe 'Fecha alta' en los datos, el informe excluye reservas creadas después de esa fecha.\n"
        "• Perímetro del periodo: los ingresos y noches se prorratean para contar solo lo que cae en el rango seleccionado."
    )
    pdf.multi_cell(0, 6, _safe_text(texto_glosario, unicode_ok))
    pdf.ln(2)
    pdf.set_font(*small_font)
    pdf.set_text_color(120, 120, 120)
    pdf.multi_cell(0, 5, _safe_text(
        "Notas: si no hay columna de 'Ingresos', se utiliza 'Precio por noche' para estimar ingresos. "
        "Los gráficos dependen de la disponibilidad de datos (canal).", unicode_ok))

    out_buf = pdf.output(dest="S")
    pdf_bytes = out_buf.encode("latin-1") if isinstance(out_buf, str) else bytes(out_buf)

    st.download_button(
        "📄 Descargar Informe PDF (PRO+)",
        data=pdf_bytes,
        file_name=f"Informe_Propietario_{mes_text.replace(' ', '_')}_PRO.pdf",
        mime="application/pdf",
        type="primary"
    )
    st.success("Informe profesional (PRO+) generado.")


def register_owner_report_option(options: List[str]) -> List[str]:
    label = "Informe de propietario (PDF)"
    return options if label in options else options + [label]
# ================== FIN BLOQUE INLINE — INFORME PROPIETARIO ==================


# ---- Selector de informes ----
with st.sidebar.expander("Informes", expanded=True):
    _owner_choice = st.selectbox("Abrir informe", ["(Ninguno)","Informe de propietario (PDF)"], key="owner_report_selector")
if _owner_choice == "Informe de propietario (PDF)":
    render_owner_report_mode()
    st.stop()
# --------------------------------
