...
# ================== BLOQUE INLINE — INFORME PROPIETARIO (PDF) ==================
...
def _kpis_fallback(
    ...
    for key in ["ingres", "revenue", "importe", "total", "aloj"]:
        cand = _find_col(df, [key], contains=True)
        if cand is None:
            continue
        if np.issubdtype(pd.to_datetime(df[cand], errors="ignore").dtype, np.datetime64):
            continue
        try:
...
def render_owner_report_mode():
    ...
    with st.sidebar:
        ...
        def _auto_detect_columns(df: pd.DataFrame) -> dict:
            guess = {"aloj": None, "ini": None, "fin": None, "alta": None, "rev": None, "price": None}
            cols = list(df.columns)
    ...
    def _kpis_from_df(
        ...
        if "Alojamiento" not in df.columns:
            raise ValueError("Selecciona la columna de alojamiento en la barra lateral.")
        if "_EntradaTmp" not in df.columns or "_SalidaTmp" not in df.columns:
            raise ValueError("Selecciona las columnas de fecha de entrada y salida en la barra lateral.")
    ...
    try:
        ...
        st.info("Añade a requirements.txt: fpdf2==2.7.9")
        return
    ...
    st.success("Informe generado. Próximo paso: insertar gráficos/heatmap en PNG.")
...
# ---- Selector rápido para abrir el Informe de Propietario ----
...
# ---- Fuente Unicode (para acentos / € / viñetas) ----
_DEJAVU_CANDIDATES = [
    "./assets/fonts/DejaVuSans.ttf",
    "./fonts/DejaVuSans.ttf",
    "./DejaVuSans.ttf",
...
def _safe_text(s: str, unicode_ok: bool) -> str:
    """Evita caracteres problemáticos si no tenemos fuente Unicode."""
    if unicode_ok:
        return s
    return (
        s.replace("•", "-").replace("–", "-").replace("—", "-")
...
def _parse_date_series(s: pd.Series) -> pd.Series:
    """Convierte a fecha (date) de forma robusta y sin timezone."""
    s2 = pd.to_datetime(s, errors="coerce", dayfirst=True)
    try:
        if getattr(s2.dt, "tz", None) is not None:
            s2 = s2.dt.tz_localize(None)
...
# --- Helpers: auto-detección de columnas ---
def _auto_detect_columns(df: pd.DataFrame) -> dict:
...
# --- KPIs con mapeo explícito y extras para gráficos ---
def _kpis_from_df(
    df_all: pd.DataFrame,
    cutoff: date,
    period_start: date,
    ...
    inventory_override: Optional[int] = None,
    filter_props: Optional[List[str]] = None,
    aloj_col: Optional[str] = None,
    ini_col: Optional[str] = None,
    fin_col: Optional[str] = None,
    alta_col: Optional[str] = None,
    ...
    # Cutoff por fecha de alta (si existe)
    if "_AltaTmp" in df.columns:
        df = df[pd.to_datetime(df["_AltaTmp"], errors="coerce") <= pd.to_datetime(cutoff)]
    ...
    # Noches disponibles del periodo
    days = (period_end - period_start).days + 1
    nights_available = max(inv * days, 0)

    # Recaudación y noches vendidas con prorrateo a rango
    nights_sold = 0
    revenue = 0.0
    agg = {}
    ...
    df_ci_window = []  # reservas cuyo check-in cae dentro del periodo (para mix de canales)
    ...
    extras = {
        "df_for_charts": df,
        "inv": inv,
        "df_ci_window": pd.DataFrame(df_ci_window) if df_ci_window else pd.DataFrame(),
    }
    tot = {
        "noches_ocupadas": nights_sold,
        "noches_disponibles": nights_available,
...
# --- Utils para gráficos y payout ---
def _build_daily_occupancy(df: pd.DataFrame, period_start, period_end, inv: int) -> pd.DataFrame:
    """Devuelve un DataFrame con índice fecha y % de ocupación diaria (0-100)."""
    days = pd.date_range(start=pd.to_datetime(period_start), end=pd.to_datetime(period_end), freq="D")
    occ = pd.Series(0.0, index=days)
    pe_exclusive = pd.to_datetime(period_end) + pd.Timedelta(days=1)

    for _, r in df.iterrows():
        r_in, r_out = pd.to_datetime(r["_in"]), pd.to_datetime(r["_out"])
        start = max(r_in, pd.to_datetime(period_start))
        end   = min(r_out, pe_exclusive)
        if end <= start:
            continue
        span = pd.date_range(start=start, end=end - pd.Timedelta(days=1), freq="D")
        occ.loc[occ.index.isin(span)] += 1.0

    inv = max(int(inv), 1)
    occ_pct = (occ / inv) * 100.0
    return pd.DataFrame({"Ocupación %": occ_pct})

def _save_matplotlib_chart(make_plot_fn, out_path: str):
    """Dibuja un gráfico con matplotlib y lo guarda en PNG."""
    try:
...
def render_owner_report_mode():
    ...
    with st.sidebar:
        ...
        # Detección + selección manual
        guess = _auto_detect_columns(raw)

        aloj_col = st.selectbox("Columna de alojamiento", options=list(raw.columns),
                                index=(list(raw.columns).index(guess["aloj"]) if guess["aloj"] in raw.columns else 0))
        ini_col  = st.selectbox("Columna fecha entrada", options=list(raw.columns),
                                index=(list(raw.columns).index(guess["ini"]) if guess["ini"] in raw.columns else 0))
        fin_col  = st.selectbox("Columna fecha salida", options=list(raw.columns),
                                index=(list(raw.columns).index(guess["fin"]) if guess["fin"] in raw.columns else 0))

        alta_col = st.selectbox("Columna fecha alta (opcional)",
                                options=["(Ninguna)"] + list(raw.columns),
                                index=(list(raw.columns).index(guess["alta"])+1 if guess["alta"] in raw.columns else 0))
        alta_col = None if alta_col == "(Ninguna)" else alta_col
        ...
        rev_col = st.selectbox("Columna ingresos (opcional)",
                               options=["(Ninguna)"] + num_cols,
                               index=(rev_default_idx+1 if num_cols and guess["rev"] in num_cols else 0))
        rev_col = None if rev_col == "(Ninguna)" else rev_col

        price_col = st.selectbox("Columna precio por noche (opcional, si no hay ingresos)",
                                 options=["(Ninguna)"] + num_cols,
                                 index=(price_default_idx+1 if num_cols and guess["price"] in num_cols else 0))
        price_col = None if price_col == "(Ninguna)" else price_col

        canal_col = st.selectbox("Columna canal (opcional)",
                                 options=["(Ninguna)"] + list(raw.columns),
    ...
    logo_path = next((lp for lp in LOGO_PATHS if os.path.exists(lp)), None)
    ...
    if logo_path:
        try:
            pdf.image(logo_path, x=160, y=18, w=30)
        except Exception:
            pass
    ...
    pdf.cell(0, 8, _safe_text(f"Propietario: {propietario}", unicode_ok), ln=1)
    pdf.cell(0, 8, _safe_text(f"Propiedad: {propiedad}", unicode_ok), ln=1)
    pdf.cell(0, 8, _safe_text(f"Generado: {pd.Timestamp.today().strftime('%d %b %Y')}", unicode_ok), ln=1)
    ...
    # ---------- Resumen Ejecutivo ----------
    pdf.add_page()
    pdf.set_text_color(*BRAND_RGB); pdf.set_font(*h2_font)
    pdf.cell(0, 8, _safe_text("Resumen ejecutivo", unicode_ok), ln=1)

    pdf.set_text_color(*DARK_RGB); pdf.set_font(*body_font)
    resumen = (
        ...
        "Este resultado refleja la combinación de demanda del periodo, estrategia de precios y mix de canales.\n\n"
        "Recomendaciones rápidas:\n"
        "- Revisar precios de alta demanda con al menos 30–45 días de antelación.\n"
        "- Mantener mínimos de estancia (LOS) óptimos en fines de semana y eventos.\n"
        "- Asegurar paridad entre canales y reforzar venta directa cuando el canal dominante cobre comisiones elevadas."
    )
    ...
    try:
        ...
        if not df_for_ch.empty:
            ...
            chart1 = os.path.join(tmpdir, "owner_daily_occ.png")
            def _plot1(plt):
                daily.plot(ax=plt.gca(), legend=False)
                plt.title("Ocupación diaria (%)")
    ...
    # 2) Mix de canales (si existe)
    try:
        if canal_col and canal_col in raw.columns:
            df_ci = extras.get("df_ci_window", pd.DataFrame())
            if not df_ci.empty and canal_col in df_ci.columns:
                mix = df_ci[canal_col].astype(str).value_counts().head(8)
                chart2 = os.path.join(tmpdir, "owner_channel_mix.png")
                def _plot2(plt):
                    mix.plot(kind="bar", ax=plt.gca())
                    plt.title("Mix de canales (reservas con check-in en el periodo)")
                    plt.xlabel("Canal"); plt.ylabel("Reservas")
    ...
    cur_y = 80 + 2*(28 + 10) + 10  # debajo de las tarjetas
    for cp in chart_paths:
        try:
            pdf.image(cp, x=15, y=cur_y, w=180)
            cur_y += 70
        except Exception:
            pass
    ...
    pdf.add_page()
    pdf.set_text_color(*BRAND_RGB); pdf.set_font(*h2_font)
    pdf.cell(0, 8, _safe_text("Resumen económico del propietario", unicode_ok), ln=1)
    pdf.set_text_color(*DARK_RGB); pdf.set_font(*body_font)

    explain = (
        "A continuación se presenta el cálculo del pago al propietario para el periodo seleccionado. "
    ...
    econ = [
        ("Ingresos brutos (€)", f"{tot['ingresos']:.2f}"),
        ("Comisión gestión (%)", f"{mgmt_pct:.1f}%"),
        ("Comisión gestión (€)", f"{payout['mgmt_fee']:.2f}"),
        ("Costes fijos (€)",     f"{payout['fixed_costs']:.2f}"),
        ("Pago propietario (€)", f"{payout['payout']:.2f}"),
    ]
    x0, y0 = 15, 60
    for i, (lab, val) in enumerate(econ):
        x = x0 + (i % 3) * 60
        y = y0 + (i // 3) * 30
        kpi_card(x, y, lab, val)
    ...
    pdf.add_page()
    pdf.set_text_color(*BRAND_RGB); pdf.set_font(*h2_font)
    pdf.cell(0, 8, _safe_text("Glosario y metodología", unicode_ok), ln=1)
    pdf.set_text_color(*DARK_RGB); pdf.set_font(*body_font)

    texto_glosario = (
        "Ocupación (%): noches vendidas / noches disponibles.\n"
        "ADR (€): ingreso medio por noche ocupada. Si no hay columna de ingresos, se aproxima con precio por noche x noches.\n"
        "RevPAR (€): ingresos / noches disponibles.\n"
        "Fecha de corte: si existe 'Fecha alta' en los datos, el informe excluye reservas creadas después de la fecha de corte.\n"
        "Perímetro del periodo: los ingresos y noches se prorratean para contar solo lo que cae dentro del rango seleccionado."
    )
    pdf.multi_cell(0, 6, _safe_text(texto_glosario, unicode_ok))

    pdf.ln(4)
    pdf.set_font(*h3_font); pdf.set_text_color(*BRAND_RGB)
    pdf.cell(0, 6, _safe_text("Notas", unicode_ok), ln=1)
    pdf.set_text_color(*DARK_RGB); pdf.set_font(*small_font)
    pdf.multi_cell(0, 5, _safe_text(
        "- Si no hay columna de 'Ingresos', el informe utiliza 'Precio por noche' para estimar ingresos.\n"
        "- Si el periodo cruza varios meses, la ocupación diaria se calcula para todo el rango.\n"
        "- Los gráficos son orientativos; pueden variar por disponibilidad de datos (canal, lead time).", unicode_ok))

    # Descargar (robusto a bytes/bytearray/str)
    out_buf = pdf.output(dest="S")
    pdf_bytes = out_buf.encode("latin-1") if isinstance(out_buf, str) else bytes(out_buf)
    ...
    st.success("Informe profesional generado.")
# ================== FIN INFORME DE PROPIETARIO — PRO ==================
