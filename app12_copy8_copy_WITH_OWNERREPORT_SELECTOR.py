...
def render_owner_report_mode():
    ...
    if raw is None or raw.empty:
        st.warning("Carga un archivo de reservas para generar el informe (barra lateral).")
        st.stop()

    with st.expander("Vista previa de datos (primeras 10 filas)", expanded=False):
    ...
    with st.sidebar:
        ...
        # Selector de columna de alojamiento
        smart_guess = None
        candidates = [
            "Alojamiento","Unidad","Apartamento","Propiedad","Listing","Unidad/Alojamiento","Nombre alojamiento",
            "ALOJAMIENTO","UNIDAD","APARTAMENTO","PROPIEDAD","LISTING"
        ]
        for c in raw.columns:
            for pat in candidates:
                if pat.lower() in str(c).lower():
                    smart_guess = c; break
            if smart_guess: break

        text_cols = [c for c in raw.columns if raw[c].dtype == "object" or str(raw[c].dtype).startswith("string")]
        if smart_guess and smart_guess not in text_cols:
            text_cols = [smart_guess] + [c for c in text_cols if c != smart_guess]

        aloj_col = st.selectbox(
            "Columna de alojamiento",
            options=text_cols if text_cols else list(raw.columns),
            index=(text_cols.index(smart_guess) if (smart_guess and smart_guess in text_cols) else 0) if (text_cols or list(raw.columns)) else 0,
            help="Selecciona la columna que identifica el alojamiento / unidad."
        )
        try:
    ...
    raw2 = raw.copy()
    if "Alojamiento" not in raw2.columns and aloj_col in raw2.columns:
        raw2 = raw2.rename(columns={aloj_col: "Alojamiento"})
