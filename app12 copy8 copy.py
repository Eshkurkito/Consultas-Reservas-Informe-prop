...
def _safe_text(s: str, unicode_ok: bool) -> str:
    if unicode_ok:
        return s
    # Fallback para evitar errores con Helvetica (sin Unicode)
    return (
        s.replace("•", "-")
         .replace("–", "-")
         .replace("—", "-")
...
def _parse_date_series(s: pd.Series) -> pd.Series:
    """
    Convierte a fecha (date) de forma robusta:
    - Acepta datetime/date/string
    - Maneja series tz-aware y tz-naive sin romper
    """
    s2 = pd.to_datetime(s, errors="coerce", dayfirst=True)
    # Quitar tz si la hubiera (sin fallar si ya es naive)
    try:
...
def _kpis_fallback(
    ...
    for key in ["ingres", "revenue", "importe", "total", "aloj"]:
        ...
        if cand and np.issubdtype(pd.to_datetime(df[cand], errors="ignore").dtype, np.datetime64):
            cand = None
        if cand is not None:
            try:
    ...
    # Filtro por cutoff (si hay fecha alta)
